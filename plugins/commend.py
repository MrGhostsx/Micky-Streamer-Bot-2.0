...
from plugins.ghostbot import is_user_joined, ghost_verification, ghost_x_verification
import os
import json
import asyncio
import logging
...
async def start(client, message):
    ...
    if msg.startswith("BATCH-"):
        ...
        if not await db.has_premium_access(user_id):
            verified = await ghost_x_verification(client, message)
            if not verified:
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [[
	    InlineKeyboardButton('💻 sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ', url='https://github.com/MrGhostsx/Micky-Streamer-Bot-2.0.git')
	],[
