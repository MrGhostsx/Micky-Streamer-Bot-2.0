...
from plugins.ghostbot import ghost_verification, is_user_allowed, is_user_joined
...
async def private_receive_handler(c: Client, m: Message):                    
    ...
    file_name = file_id.file_name if file_id.file_name else f"MrGhost_File_{int(time.time())}.mkv"
    file_size = get_size(file_id.file_size)

    if not await db.has_premium_access(user_id):
        verified = await ghost_verification(c, m)
        if not verified:
            return
    ...
    try:
        ...
        stream = f"{URL}watch/{forwarded.id}/MrGhost_File_{int(time.time())}.mkv?hash={hash_str}"
