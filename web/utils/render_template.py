...
import logging
import aiohttp
from web.utils.Template import mrghost_template
from info import *
from web.server import Webghostbot
...
async def render_page(id: str, secure_hash: str, src: str = None) -> str:
    ...
    try:
        file = await Webghostbot.get_messages(int(BIN_CHANNEL), int(id))
        file_data = await get_file_ids(Webghostbot, int(BIN_CHANNEL), int(id))
    ...
    if tag in ["video", "audio"]:
        template_file = os.path.join("web", "template", "webghost.html")
    else:
    ...
    file_name = file_data.file_name.replace("_", " ") if file_data.file_name else f"MrGhost_File_{id}.mkv"
    ...
    return template.render(
        ...
        template_ne=mrghost_template.NAME,
        disclaimer=mrghost_template.DISCLAIMER,
        report_link=mrghost_template.REPORT_LINK,
        colours=mrghost_template.COLOURS,
                                    )
#Dont Remove My Credit @Tech_Shreyansh
#This Repo Is By @SmartEdith_Bot 
# For Any Kind Of Error Ask Us In Support Group @Tech_Shreyansh2
